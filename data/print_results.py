...
def print_results(results_dic, results_stats_dic, model, 
    ...
    if print_incorrect_dogs and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']):
        ...
        for filename, info in results_dic.items():
            is_dog, classified_as_dog = info[3:5]
            if is_dog != classified_as_dog:
                print(f"  Pet Image: {info[0]}")
                print(f"  Classifier Label: {info[1]}\n")

    # Print incorrect breed classifications if requested
    if print_incorrect_breed and (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']):
        print("\nIncorrect Dog Breed Classifications:")
        for filename, info in results_dic.items():
            is_match, is_dog, classified_as_dog = info[2:5]
            if is_dog and classified_as_dog and not is_match:
                print(f"  Pet Image: {info[0]}")
                print(f"  Classifier Label: {info[1]}\n")
